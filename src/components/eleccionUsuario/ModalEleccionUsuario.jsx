...
import { createEleccionUsuario, getEleccionState, getUsuarioAllState } from "../../services";
...
const ModalEleccionUsuario = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await getUsuarioAllState("Activo");
        const options = data.data.map((usuario) => ({
          value: usuario.id_usuario,
          label: `${usuario.nombre_usuario} ${usuario.lastname_usuario} - ${usuario.cedula_usuario}`
        }));
  ...
  const handleSubmit = async (evt) => {
    ...
    if (selectedUsuarios.length > 0) {
      const payload = {
        ...
        usuarios_id: selectedUsuarios.at(-1).map((selected) => selected.value),
        estado_id: 1
      };
      try {
        await createEleccionUsuario(payload).then((res) => {
          if (validateStatus(res.status)) {
            throw Error();
          } else {
            NotificationSucces("Usuarios agregados correctamente");
            setTimeout(() => {
              onClose();
            }, 2000);
          }
          ...
          <div className="col-span-2">
            {stateOptions.length > 0 && (
              <Select
                ...
                onChange={(selected) => {
                  setSelectedUsuarios((prev) => [...prev, selected]);
                }}
              />
            )}
          </div>
...
export default ModalEleccionUsuario;
