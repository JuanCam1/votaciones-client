...
const ModalVote = ({ handleReset,setIsValidate, selected, idVotante, isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      id_eleccione_votante: selected.id_eleccione_votante,
      candidato_id: candidato_id,
      eleccion_id: selected.eleccion_id,
      votante_id: idVotante,
    ...
    try {
      await createVotoVotante(payload).then(async (res) => {
        if (res.status === 202) {
          NotificationSucces("Ha votado Correctamente");

          await eleccionesIsVotante(idVotante).then((res) => {
            if (res.status === 202) {
    ...
    >
      <div className="border shadow-md p-3 rounded-md">
        <div className="flex flex-col justify-center items-center py-3">
          ...
          <p>
            {`Fecha Finalización: ${format(new Date(selected.fecha_fin_eleccion), "d 'de' MMMM 'de' yyyy", { locale: es })}`}{"-"}
